# MP-23: Next.js Project Setup + API Client

## Goal
Initialize the Next.js 14 frontend project with Tailwind CSS and create the API client module (`lib/api.ts`) with all TypeScript interfaces, fetch functions, and polling logic.

## Dependencies
- Node.js 18+ and npm installed
- MP-18/19 completed (backend API endpoints defined)
...
```bash
cd dpdp-copilot
npx create-next-app@14 frontend --typescript --tailwind --eslint --app --src-dir=false --import-alias="@/*" --no-git
cd frontend
...
export async function pollAnalysis(
  ...
  while (attempt < maxAttempts) {
    const res = await fetch(
      `${API_BASE}/api/analyze/${analysisId}/status`,
      { credentials: "include" }
    );
...
### 3. Create `next.config.js` update

Ensure `next.config.js` (or `next.config.mjs`) allows the backend API:

```
FILE: frontend/next.config.mjs (update if exists, create if not)
```
...
# Test 2: TypeScript compiles (api.ts has no errors)
npx tsc --noEmit lib/api.ts 2>&1 || echo "Check TS errors above"

# Test 3: Dev server starts
echo "Run 'npm run dev' and verify http://localhost:3000 loads"
```
