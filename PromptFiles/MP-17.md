# MP-17: Analysis Pipeline Orchestrator

## Goal
Create the main pipeline that orchestrates the full analysis flow: classify → map → gap analysis → score → save. Runs as a background task. Updates DB status at each step. Falls back gracefully on LLM failure.

## Dependencies
- MP-02 completed (`db/database.py`)
- MP-14 completed (`analysis/classifier.py`)
- MP-15 completed (`analysis/mapper.py`)
- MP-16 completed (`analysis/gap_analyzer.py`)
- MP-12 completed (`analysis/fallback.py`)
- MP-04 completed (`llm/client.py` with `LLMError`)
...
```
FILE: backend/app/analysis/pipeline.py
```
...
# Test 1: Score calculation — all missing
gaps = [
...
assert pct == 0
assert risk == 'high'
print(f'Test 1 (all missing): {pct}%, {risk}. PASSED')

# Test 2: Score calculation — mixed
gaps = [
...
pct, risk = _calculate_score(gaps)
assert 50 <= pct <= 70  # 2 compliant + 0.5 partial / 4 = ~62%
print(f'Test 2 (mixed): {pct}%, {risk}. PASSED')

# Test 3: Score calculation — all compliant
gaps = [
    {'status': 'compliant', 'severity': 'low'},
    {'status': 'compliant', 'severity': 'low'},
]
...
assert pct == 100
assert risk == 'low'
print(f'Test 3 (all compliant): {pct}%, {risk}. PASSED')

# Test 4: Fallback — no policy
gaps = _fallback_gap_report(
...
print('Test 4 (fallback no policy): PASSED')

# Test 5: Fallback — with policy
gaps = _fallback_gap_report(
    [{'category': 'consent', 'act_sections': ['Section 6']}],
    'We have a privacy policy that covers data collection and usage...' * 5
)
assert gaps[0]['status'] == 'partial'
print('Test 5 (fallback with policy): PASSED')

# Test 6: Serialize Pydantic models
from app.models.schemas import DataClassification, RiskLevel
items = [
    DataClassification(field_name='email', categories=['identifiers'], risk_level=RiskLevel.MEDIUM, reasoning='PII', confidence=0.9)
]
result = _serialize(items)
import json
parsed = json.loads(result)
assert len(parsed) == 1
assert parsed[0]['field_name'] == 'email'
print('Test 6 (serialize): PASSED')

# Test 7: Empty gaps
pct, risk = _calculate_score([])
assert pct == 0
assert risk == 'high'
print('Test 7 (empty): PASSED')

print('MP-17 PASSED')
"
```

**Expected output:** All 7 tests pass, ending with `MP-17 PASSED`.
